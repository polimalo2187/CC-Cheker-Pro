...
def add_referral(new_user_id, ref_code):
    referrer = None
    ...
    if not referrer or referrer["user_id"] == new_user_id:
        return  # Código inválido o el usuario se refiere a sí mismo
...
def start(update: Update, context: CallbackContext):
    ...
    if not user:
        ...
        # Revisar si se pasó código de referido
        args = context.args
        if args:
            add_referral(user_id, args[0])
